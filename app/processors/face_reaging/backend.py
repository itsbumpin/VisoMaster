...
class _SAMBackend:
    ...
    def _build_conditions(
        ...
        current_norm = torch.tensor([max(0.0, min(current_age, 100.0)) / 100.0], dtype=dtype, device=device)
        target_norm = torch.tensor([max(0.0, min(target_age, 100.0)) / 100.0], dtype=dtype, device=device)
        if batch_size > 1:
    ...
    def __call__(
        ...
        try:
            ...
            if batched.size(0) != 1:
                batched = batched[:1]
            batched = batched.to(device=self.device, dtype=torch.float32)
            current_cond, target_cond = self._build_conditions(
