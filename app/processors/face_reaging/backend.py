...
LOG = logging.getLogger("FaceReaging")
if not LOG.handlers:
    handler = logging.StreamHandler()
    handler.setFormatter(logging.Formatter("[FaceReaging] %(levelname)s: %(message)s"))
...
class _FaceAgingGanBackend:
    ...
    def _load(self) -> None:
        if self.model_path is None or not self.model_path.exists():
            ...
            self._log("Missing Face Aging GAN checkpoint", level=logging.WARNING)
            return
        ...
        try:
            ...
            self._log(f"Unable to read checkpoint {self.model_path}: {load_error}", level=logging.WARNING)
            return
        ...
        if not state_dict:
            ...
            self._log(
                f"Checkpoint format not recognised ({type(payload).__name__}) from {self.model_path}",
        ...
        if weight_key is None:
            ...
            self._log("Unable to infer generator input shape from checkpoint", level=logging.WARNING)
            return
        ...
        if first_weight.ndim != 4:
            ...
            self._log("Unexpected generator weight dimensions in checkpoint")
            return
        ...
        if self.status.is_ready:
            self._log(f"Loaded checkpoint {self.model_path}")
        else:
            self._log(f"Loaded with mismatched keys (missing={len(missing)}, unexpected={len(unexpected)})")
...
class _SAMBackend:
    ...
    def __init__(self, device: str | torch.device) -> None:
        ...
        self._forward_debug_logged = False
        self._resolve_checkpoint()
        self._load()
    ...
    def _resolve_checkpoint(self) -> None:
        ...
        for candidate in candidates:
            ...
            if path.exists():
                self.model_path = path
                self._log(f"Found checkpoint {path}")
                return
        ...
        if checkpoints:
            self.model_path = checkpoints[0]
            self._log(f"Found checkpoint {self.model_path}")
        else:
            self.model_path = None
            self._log("No SAM checkpoint found", level=logging.WARNING)
    ...
    def _try_load_script(self) -> bool:
        ...
        self._log(f"Loaded TorchScript checkpoint {self.model_path}")
        return True
    ...
    def _load(self) -> None:
        if self.model_path is None or not self.model_path.exists():
            ...
            self._log("Missing SAM checkpoint", level=logging.WARNING)
            return
        ...
        try:
            ...
            self._log(f"Unable to read SAM checkpoint {self.model_path}: {load_error}", level=logging.WARNING)
            return
        ...
        if module is None:
            ...
            self._log(
                f"SAM checkpoint format not recognised ({type(payload).__name__}) from {self.model_path}",
                level=logging.WARNING,
            )
            return
        ...
        try:
            ...
            self._log("SAM module does not support device transfer", level=logging.WARNING)
            return

        module.eval()
        self.model = module
        self.status = _BackendStatus(is_ready=True, error=None)
        self._log(f"Loaded SAM module from {self.model_path}")
    ...
    def __call__(
        ...
        try:
            ...
            successful_args: tuple[Any, ...] = ()
            successful_kwargs: Dict[str, Any] = {}
            with torch.no_grad():
                for idx, (args, kwargs) in enumerate(attempts):
                    try:
                        ...
                        if output is not None:
                            successful_attempt = idx
                            successful_args = args
                            successful_kwargs = kwargs
                            break
            ...
            result = torch.clamp((output + 1.0) * 0.5, 0.0, 1.0)
            self.status = _BackendStatus(is_ready=True, error=None)
            if not self._forward_debug_logged and successful_attempt is not None:
                arg_types = ", ".join(type(arg).__name__ for arg in successful_args) or "none"
                kwarg_keys = ", ".join(successful_kwargs.keys()) or "none"
                self._log(
                    f"Forward succeeded on attempt #{successful_attempt + 1} (arg types={arg_types}; kwargs={kwarg_keys})"
                )
                self._forward_debug_logged = True
            return result
        except Exception as runtime_error:  # pragma: no cover - best effort fallback
            self.status = _BackendStatus(is_ready=False, error=str(runtime_error))
            self._log(f"Runtime error during forward pass: {runtime_error}", level=logging.WARNING)
            return None
