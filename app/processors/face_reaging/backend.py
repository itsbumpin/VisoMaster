...
class _FaceAgingGanBackend:
    ...
    def _log(self, message: str) -> None:
        print(f"[FaceReaging][GAN] {message}")
    ...
    def _load(self) -> None:
        if self.model_path is None or not self.model_path.exists():
            ...
            self._log("Missing Face Aging GAN checkpoint")
            return
        ...
        try:
            ...
            self._log(f"Unable to read checkpoint {self.model_path}: {load_error}")
            return
        ...
        if not state_dict:
            ...
            self._log(f"Checkpoint format not recognised ({type(payload).__name__}) from {self.model_path}")
            return
        ...
        if weight_key is None:
            ...
            self._log("Unable to infer generator input shape from checkpoint")
            return
...
class _SAMBackend:
    ...
    def _log(self, message: str) -> None:
        print(f"[FaceReaging][SAM] {message}")
    ...
    def _resolve_checkpoint(self) -> None:
        ...
        if checkpoints:
            ...
            self.model_path = None
            self._log("No SAM checkpoint found")
    ...
    def _load(self) -> None:
        if self.model_path is None or not self.model_path.exists():
            ...
            self._log("Missing SAM checkpoint")
            return
        ...
        try:
            ...
            self._log(f"Unable to read SAM checkpoint {self.model_path}: {load_error}")
            return
        ...
        if module is None:
            ...
            self._log(f"SAM checkpoint format not recognised ({type(payload).__name__}) from {self.model_path}")
            return
        ...
        try:
            ...
            self._log("SAM module does not support device transfer")
            return
    ...
    def __call__(
        ...
        try:
            ...
            self.status = _BackendStatus(is_ready=False, error=str(runtime_error))
            self._log(f"Runtime error during forward pass: {runtime_error}")
            return None
