...
class FaceEditors:
    def __init__(self, models_processor: 'ModelsProcessor'):
        ...
        self._face_reaging_logger = logging.getLogger("FaceReaging")
        self._face_reaging_logger.setLevel(logging.INFO)
    ...
    def apply_face_reaging(self, img: torch.Tensor, parameters: dict) -> torch.Tensor:
        ...
        if not self._face_reaging_debug_logged.get(model_key, False):
            ...
            self._face_reaging_logger.info(
                "Using %s (key=%s) current=%.1f, target=%.1f, strength=%.2f, blend=%.2f%s",
                selected_label,
                model_key,
                current_age,
                target_age,
        ...
        if backend_face is None:
            if not self._face_reaging_warning_emitted.get(model_key, False):
                ...
                self._face_reaging_logger.warning("%s unavailable: %s", selected_label, reason)
